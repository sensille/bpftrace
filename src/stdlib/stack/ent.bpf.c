...
} ent_expressions SEC(".maps");

struct {
	__uint(type, BPF_MAP_TYPE_PERCPU_ARRAY);
	__uint(max_entries, 1);
	__type(key, u32);
	__type(value, struct ent_state);
} ent_state_scrach SEC(".maps");
...
{
	struct offsetmap *om = bpf_map_lookup_elem(&ent_offsetmaps, &offsetmap_id);
	if (om == NULL) {
...
unwind_step(int ix)
{
  ...
  u32 zero = 0;
  struct ent_state *s = bpf_map_lookup_elem(&ent_state_scrach, &zero);
  if (s == NULL) {
	 ...
	 */
	struct mapping *m = bpf_map_lookup_elem(&ent_mappings, &s->tgid);
	if (m == NULL) {
  ...
  int cft_id;
	...
	struct cft *cf = bpf_map_lookup_elem(&ent_cfts, &cft_id);
	if (cf == NULL) {
		LOG("cft not found");
		return 1;
	}
	...
	if (cf->cfa.rtype == CFA_RULE_UNINITIALIZED) {
		...
		cfa = regs_o[r] + o;
		bpf_printk("  CFA = r%d (%lx) + %lld = %lx", r, regs_o[r], o);
  ...
  if (s->current_reg_set < 2) /* bound check */
	...
	bpf_printk("After unwind step: next PC %lx", regs_n[RIP]);
#if 0
	for (int reg = 0; reg < NUM_REGISTERS; ++reg) {
		if (*regs_vn & (1 << reg)) {
...
long __ustack_ent(void *ctx, u64 *buf, u32 buf_size)
{
  ...
  if (buf_size < 8) {
    bpf_printk("buffer too small");
    return 0;
  }

	*buf++ = s->regs_map[RIP];
	// pre-5.17 without bpf_loop
	for (int i = 1; i < MAX_STACK_FRAMES && i < buf_size / sizeof(u64); ++i) {
	  [[clang::noinline]] if (unwind_step(i) != 0)
			goto done;
    ulong off = s->current_reg_set == 1 ? NUM_REGISTERS : 0;
    *buf++ = s->regs_map[RIP + off];
		bpf_printk("frame %d: RIP %lx RSP %lx", i,
